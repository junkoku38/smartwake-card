...
let SmartWAKECard = class SmartWAKECard extends i {
    ...
    render() {
      ...
      <ha-card class="card">
        ...
        ${prochain ? b `<div class="next-alarm">⏰ Prochain : ${prochain}</div>` : A}

        <!-- Jours -->
        <div class="section-title">Jours</div>
        <div class="chips">
          ${JOURS_LABELS.map((label, i) => b `
            <span class="chip ${jours.includes(i) ? "on" : ""}">${label}</span>
          `)}
        </div>

        ${this._config.show_context ? b `
          <!-- Contexte -->
          ...
          <div class="chips">
            ...
            ${this._isOn(`binary_sensor.${base}_weekend`) ? b `<span class="chip context on">🏖️ Weekend</span>` : A}
            ${this._isOn(`binary_sensor.${base}_jour_ferie`) ? b `<span class="chip context on">🎉 Férié</span>` : A}
            ${this._isOn(`binary_sensor.${base}_vacances_scolaires`) ? b `<span class="chip context on">🏫 Vacances sco</span>` : A}
          </div>
        ` : A}
